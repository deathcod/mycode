...
public:
	...
	void bfs_using_adjacancy_matrix(int source, std::vector<int> G[8], int n);
...
void graph_traversal :: bfs_using_adjacancy_matrix(int source, std::vector<int> G[8], int n)
{
	//Creating the adjacancy matrix from adjacancy list, I know this is a wrong coding practice
	int G_matrix[n][n] = {0};
	for (int i = 1; i <= n; ++i)
	{
	...
	while(!q.empty())
	{
		...
		for (int i = 1; i <= n; ++i)
		{
			if(G_matrix[f][i] and visited[i] == 0)
			{
				visited[i] = 1;
				q.push(i);
			}
...
int main(int argc, char const *argv[])
{
	...
	cout << "BFS using adjacancy matrix\n";
	x->bfs_using_adjacancy_matrix(1, G, 7);
